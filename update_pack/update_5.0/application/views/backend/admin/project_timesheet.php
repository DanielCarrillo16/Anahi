				...
				<tr>
					...
					<td>
						<a href="#" class="btn btn-default btn-xs tooltip-primary" data-toggle="tooltip"  data-placement="left" 
	                        ...
	                        onclick="confirm_modal('<?php echo site_url('admin/project_timer/delete/' . $row2['project_timesheet_id']); ?>', '<?php echo site_url('admin/reload_projectroom_timer' . '/' . $row['project_code']); ?>');">
...
<script type="text/javascript">
	...
	var post_refresh_url    =   '<?php echo site_url('admin/reload_projectroom_timer/' . $project_code);?>';
	/* function for updating timer status // 0=stopped,1=running*/
    function update_timer_status(timer_status, project_id)
    {
    	
        $.ajax({
            url: '<?php echo site_url('admin/project_timer/edit/'); ?>' + timer_status + '/' + project_id,
