...
<tbody>
    ...
    foreach ($projects as $row):
        ...
        <tr>
            ...
            <td><?php
                $influencer = $this->db->get_where('influencer', array('influencer_id' => $row['influencer_id']));
                $client = $this->db->get_where('client', array('client_id' => $row['client_id']));
                if ($client->num_rows() > 0){
                    echo $client->row()->name;
                } elseif($influencer->num_rows() > 0){
                    echo $influencer->row()->name;
                }
                ?>
            </td>
            <td>
            ...
            <td>
                ...
                <a id="archive_link" class="btn btn-default tooltip-primary" data-toggle="tooltip" data-placement="top"
                   ...
                   onclick="mark_archived('<?php echo site_url('admin/project/mark_as_archive/' . $row['project_code']); ?>', '<?php echo site_url('admin/reload_project_list'); ?>');">
